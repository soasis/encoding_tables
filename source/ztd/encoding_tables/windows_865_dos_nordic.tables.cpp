...
ZTD_EXTERN_C_I_ bool ztdc_windows_865_dos_nordic_index_to_code_point(
     size_t __lookup_index_pointer, uint_least32_t* __p_code_point) ZTD_USE(ZTD_NOEXCEPT_IF_CXX) {
	auto __val = ztd::et::windows_865_dos_nordic_index_to_code_point(__lookup_index_pointer);
	if (__val) {
...
ZTD_EXTERN_C_I_ bool ztdc_windows_865_dos_nordic_code_point_to_index(
     uint_least32_t __lookup_code_point, size_t* __p_index) ZTD_USE(ZTD_NOEXCEPT_IF_CXX) {
	auto __val = ztd::et::windows_865_dos_nordic_code_point_to_index(__lookup_code_point);
	if (__val) {
		*__p_index = *__val;
		return true;
	}
