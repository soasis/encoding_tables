...
ZTD_EXTERN_C_I_ bool ztdc_windows_1256_index_to_code_point(
	...
	auto __val = ztd::et::windows_1256_index_to_code_point(__lookup_index_pointer);
	if (__val) {
...
ZTD_EXTERN_C_I_ bool ztdc_windows_1256_code_point_to_index(
     uint_least32_t __lookup_code_point, size_t* __p_index) ZTD_NOEXCEPT_IF_CXX_I_ {
	auto __val = ztd::et::windows_1256_code_point_to_index(__lookup_code_point);
	if (__val) {
		*__p_index = *__val;
		return true;
	}
